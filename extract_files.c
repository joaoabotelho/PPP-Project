...
void get_int_file_Wfscan(FILE *file, char look, int *numb) {
...
void get_string_file_Wfscan(FILE *file, char look, char **change) {
    char *string = (char*) malloc (CHAR_SIZE * sizeof(char));
...
void ex_classes(Classes_list head){
    ...
    while(fscanf(classes, "%s", string) != EOF) { 
            ...
            get_string_file_Wfscan(classes, ';', &temp.teacher);
...
void ex_students(Student_list head) {
    ...
    while(fscanf(students, "%s", string) != EOF) {
        ...
        get_string_file_Wfscan(students, ',', &temp.course);

        temp.year = (int *) malloc (sizeof(int));
        get_int_file_Wfscan(students, ',', temp.year); 

        temp.regime = (char *) malloc (CHAR_SIZE * sizeof(char));
        get_string_file_Wfscan(students, ';', &temp.regime);
        
        append_student_wOrder(&head, temp);
    }
    fclose(students);
}
...
void ex_exams(Exams_list head, Classes_list *classes, Student_list students) {
    ...
    while(fscanf(exams, "%s", string) != EOF) {
        ...
        *p = 0;
        ...
        get_int_file_Wfscan(exams, '/', &temp.date.day); 
        get_int_file_Wfscan(exams, '/', &temp.date.month); 
        get_int_file_Wfscan(exams, ',', &temp.date.year); 
  
        get_int_file_Wfscan(exams, ':', &temp.time.hour); 
        get_int_file_Wfscan(exams, ',', &temp.time.minutes); 
  
        get_int_file_Wfscan(exams, ':', &temp.duration.hour); 
        get_int_file_Wfscan(exams, ',', &temp.duration.minutes); 
  
        get_int_file_Wfscan(exams, ':', &temp.final.hour); 
        get_int_file_Wfscan(exams, ';', &temp.final.minutes); 
  
        get_int_file_Wfscan(exams, ';', &n); 
         
        temp.students_submited = create_students_list();
        for(i = 0; i < n; i++) {
            copy = students;
            get_int_file_Wfscan(exams, ',', &student_num); 

            while((*copy->data.numb != student_num) && (copy->next != NULL))
                copy = copy->next;
            append_student_wOrder(&temp.students_submited, copy->data); 
        }
        ...
        for(i = 0; i < n; i++) {
            ...
            get_string_file_Wfscan(exams, '.', &p);
            rooms.letter = *p;

            get_int_file_Wfscan(exams, '.', &rooms.floor); 
            get_int_file_Wfscan(exams, ';', &rooms.room); 
...
void ex_student_exams(Exams_list exams, Student_list students, Student_exams_list head) {
    ...
    while(fscanf(st_exams, "%s", string) != EOF) {
        ...
        append_student_exams_wOrder(&head, curr_st->data);

        get_int_file_Wfscan(st_exams, ';', &n);
        for(i = 0; i < n; i++) {
            get_int_file_Wfscan(st_exams, ',', &id_exam);
